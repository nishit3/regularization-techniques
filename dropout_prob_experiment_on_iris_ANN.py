...
class MultiClassClassifier(nn.Module):
    ...
    def __init__(self, p):
        ...
        self.hidden_layers = []
        for i in range(1):
            self.hidden_layers.append(nn.Linear(12, 12))
...
dropout_rates = np.linspace(0, 0.95, 10)
train_accuracies = []
test_accuracies = []

for i, dropout_rate in enumerate(dropout_rates):

    classifier = MultiClassClassifier(dropout_rate)
...
plt.plot(dropout_rates, train_accuracies, label='train')
plt.plot(dropout_rates, test_accuracies, label='test')
plt.legend()
