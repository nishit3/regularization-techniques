...
l2lambda_values = np.linspace(0, 0.1, 10)
train_accuracies = []
test_accuracies = []

for i, l2lambda_val in enumerate(l2lambda_values):

    classifier = MultiClassClassifier()
    loss_fun = nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(params=classifier.parameters(), lr=learning_rate, weight_decay=l2lambda_val)
    train_per_epoch = []
    test_per_epoch = []
...
plt.plot(l2lambda_values, train_accuracies, label='train')
plt.plot(l2lambda_values, test_accuracies, label='test')
plt.legend()
